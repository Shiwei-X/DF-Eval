...
class ChronosDataset(IterableDataset, ShuffleMixin):
    ...
    def preprocess_entry(self, entry: dict, mode: str) -> dict:
        ...
        entry["target"] = np.asarray(entry["target"], dtype=self.np_dtype) # 使用新增需要注释
...
def main(
    ...
    print("training_data_paths before ast.literal_eval:", training_data_paths)
    ...
    # ✅ 修改：根据 training_data_paths 提取 domain 名作为输出目录名
    ds_name = Path(training_data_paths[0]).parts[-2]  # 提取数据集名称
    output_dir = Path(output_dir) / Path(f"{ds_name}_{prediction_length}_small")  # 不使用编号，直接使用 ds_name 作为文件夹名
    ...
    train_datasets = [
        Filter(
            ...
            FileDataset(path=Path(data_path), freq="h"), # youwenti-----------------------------------------------------------------------------------------------
        )
